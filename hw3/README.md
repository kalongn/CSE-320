...
which are organized as singly linked lists accessed in LIFO fashion.  Each quick list
...
```c
    +-----------------------------------------------------------+---------+---------+---------+ <- header
    |                                       block_size          |prv alloc|  alloc  |in qklst |
...
- The `block_size` field gives the number of bytes for the **entire** block (including header/footer,
  ...
  to be preserved.
- The `alloc` bit (bit 1, mask 0x2) is a boolean. It is 1 if the block is allocated and 0 if it is free.
...
> You may create other functions if you choose, but only the specified functions
> `sf_malloc`, `sf_free`, `sf_realloc`, `sf_fragmentation`, and `sf_utilization`
> will be called from the tests used for grading.  As usual, **no** functions other
> than `main()` should be put in `main.c`.
 ...
 *
 * @return  the current amount of internal fragmentation, defined to be
 * one minus the ratio of the total amount of payload to the total size
 * of allocated blocks.  If there are no allocated blocks, then the returned
 * value should be 0.0.
 */
...
```
    ...
    +-----------------------------------------------------------+---------+---------+---------+ <- header
    |                                       block_size          |prv alloc|  alloc  |in qklst |
    |                                  (3 LSB's implicitly 0)   | (0/1)   |   (1)   |  (0/1)  |
    |                                        (1 row)            | 1 bit   |  1 bit  |  1 bit  |
    +-----------------------------------------------------------+---------+---------+---------+ <- (aligned)
    |                                                                                         |
    |                                   Payload and Padding                                   |
    ...
    NOTE: The actual stored header and footer are obfuscated by bitwise XOR'ing with MAGIC.
...
```c
    ...
    +-----------------------------------------------------------+---------+---------+---------+ <- header
    |                                       block_size          |prv alloc|  alloc  |in qklst |
    |                                  (3 LSB's implicitly 0)   |  (1)    |   (1)   |  (0/1)  | prologue
    |                                        (1 row)            | 1 bit   |  1 bit  |  1 bit  |
    +-----------------------------------------------------------+---------+---------+---------+ <- (aligned)
    |                                                                                         |
    |                                        Padding                                          |
    ...
    +-----------------------------------------------------------+---------+---------+---------+ <- header
    |                                       block_size          |prv alloc|  alloc  |in qklst |
    |                                  (3 LSB's implicitly 0)   |  (1)    |  (0/1)  |  (0/1)  | first block
    |                                        (1 row)            | 1 bit   |  1 bit  |  1 bit  |
    +-----------------------------------------------------------+---------+---------+---------+ <- (aligned)
    |                                                                                         |
    |                                   Payload and Padding                                   |
    ...
    +-----------------------------------------------------------+---------+---------+---------+ <- footer
    |                                       block_size          |prv alloc|  alloc  |in qklst |
    |                                  (3 LSB's implicitly 0)   |  (1)    |  (0/1)  |  (0/1)  |
    |                                        (1 row)            | 1 bit   |  1 bit  |  1 bit  |
    +-----------------------------------------------------------+---------+---------+---------+
    |                                                                                         |
    |                                                                                         |
    ...
    +-----------------------------------------------------------+---------+---------+---------+ <- header
    |                                       block_size          |prv alloc|  alloc  |in qklst |
    |                                  (3 LSB's implicitly 0)   |  (0/1)  |   (1)   |   (0)   | epilogue
    |                                        (1 row)            | 1 bit   |  1 bit  |  1 bit  |
    +-----------------------------------------------------------+---------+---------+---------+ <- heap end
                                                                                                   (aligned)
```
...
- After XOR'ing the stored header with `MAGIC`:
    ...
    * The `prev_alloc` field in the header is 0, indicating that the previous
      block is free, but the `alloc` field of the previous block footer is not 0.
